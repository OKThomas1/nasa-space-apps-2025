...
const handler = async (req: Request, res: Response) => {
    ...
    const dir = `./data/tempo/${z9}/${x9}/${y9}`

    if (checkIfJobIsAlreadyRunning(dir)) {
        await waitForJob(dir)
    } else if (!fs.existsSync(join(dir, "combined.tif"))) {
        const success = addJobToQueue(dir, async () => {
            fs.mkdirSync(`./data/tempo/${z9}/${x9}/${y9}`, { recursive: true })
            ...
            await Promise.all(
                urls.map(async (data) => {
                    ...
                    form.append("time", "1690989120000,1758056640000")

                    const response = await fetch(data.url, { method: "POST", body: form })
                    const blob = Buffer.from(await response.arrayBuffer())

                    fs.writeFileSync(`./data/tempo/${z9}/${x9}/${y9}/${data.name}.tif`, blob)
                })
            )
            ...
            //     await execAsync(
            //         `gdalwarp -t_srs EPSG:3857 -tr 9.554628535647032 9.554628535647032 -te ${west} ${south} ${east} ${north} -r bilinear -of GTiff ${join(dir, `${name}.tif`)} ${join(dir, `${name}_aligned.tif`)}`
            //     ).catch(console.error)
            // }
            ...
            // console.time("vrt")
            // await execAsync(
            //     `gdalbuildvrt -separate ${join(dir, "tempo_3band.vrt")} ${join(dir, "no2_aligned.tif")} ${join(dir, "hcho_aligned.tif")} ${join(dir, "o3_aligned.tif")}`
            // )
            // console.timeEnd("vrt")
            ...
            await execAsync(
                `gdal_translate ${join(dir, "tempo_3band.vrt")} ${join(dir, "tempo_3band.tif")} -of GTiff`
            )
            console.timeEnd("translate")
            console.time("cog")

            await execAsync(
                `gdal_translate ${join(dir, "tempo_3band.tif")} ${join(dir, "combined.tif")} -of COG -co COMPRESS=LZW -co NUM_THREADS=ALL_CPUS -co RESAMPLING=BILINEAR -co OVERVIEWS=IGNORE_EXISTING -co BLOCKSIZE=256 -co TILING_SCHEME=GoogleMapsCompatible -co ZOOM_LEVEL=14 -co OVERVIEW_COUNT=5`
            )
    ...
    const tiff = new SourceFile(`./data/tempo/${z9}/${x9}/${y9}/combined.tif`)
